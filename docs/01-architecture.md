...
| `agent_gateway/tools/workspace.py` | workspace-write 判定：写路径校验、macOS 命令包装 | 无 |
| `agent_gateway/tools/shell.py` | `ShellRunner`：按平台执行命令，同步与异步两种入口，超时与取消杀进程树 | workspace, windows_sandbox |
| `agent_gateway/tools/windows_sandbox.py` | Windows 受限令牌沙箱：能力 SID、ACE、令牌、Job Object、管道读取 | 无（pywin32） |
